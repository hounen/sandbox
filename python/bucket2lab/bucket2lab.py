...
GITBKT_BASE_URL="https://gitbucket/Sample"
GITLAB_BASE_URL="https://gitlab/Sample"
...
for repo in repo_list:
    # remote repository
    # print(os.getcwd())
    command = "git clone --mirror #{GITBKT_BASE_URL}/#{repo}.git"
    subprocess.call(command) #, shell=True)
    os.chdir("#{repo}.git")
    command = "git push --mirror #{GITLAB_BASE_URL}/#{repo}.git"
    subprocess.call(command)
    os.chdir("..")

    # wiki repository
    command = "git clone --mirror #{GITBKT_BASE_URL}/#{repo}.wiki.git"
    subprocess.call(command)
    os.chdir("#{repo}.git")
    command = "git push --mirror #{GITLAB_BASE_URL}/#{repo}.wiki.git"
    subprocess.call(command)
    os.chdir("..")
