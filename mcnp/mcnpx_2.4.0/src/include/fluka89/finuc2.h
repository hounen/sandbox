!     $Id: finuc2.h,v 1.4 2002/03/27 18:14:07 gwm Exp $                       !/*fi          1*/
!=== finuc2 ===========================================================*        !/*fi          2*/
!                                                                               !/*fi          3*/
!----------------------------------------------------------------------*        !/*fi          4*/
!     last change on 5 april 1990 by   alfredo ferrari, infn - milan   *        !/*fi          5*/
!     include file: finuc2 copy                  created 26/11/86 by pa*        !/*fi          6*/
!     note: if changed change finuc copy also                          *        !/*fi          7*/
!     changes: none                                                    *        !/*fi          8*/
...
!        irn       = number of secondaries                             *        !/*fi         14*/
!        itrn(ip)  = type of the secondary ip                          *        !/*fi         15*/
!        cxrn(ip)  = direction cosine of the secondary ip              *        !/*fi         16*/
!                    with respect to x-axis                            *        !/*fi         17*/
!        cyrn(ip)  = direction cosine of the secondary ip              *        !/*fi         18*/
!                    with respect to y-axis                            *        !/*fi         19*/
!        czrn(ip)  = direction cosine of the secondary ip              *        !/*fi         20*/
!                    with respect to z-axis                            *        !/*fi         21*/
!        elr(ip)   = kinetic (or total) energy of secondary ip         *        !/*fi         22*/
...
!                                                                               !/*fi         31*/
      ...
      common /finuc/ cxrn(mxp), cyrn(mxp), czrn(mxp), elr(mxp), plr(mxp)        !/*fi         33*/
     & , wei(mxp), tv, tvcms, tvrecl, tvheav, np2, irn, itrn(mxp)               !/*fi         34*/
