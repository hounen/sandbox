...
C-----------------------------------------------------------------------
     ...
     &         *SIXJ_D(0.5D0,ST12,S_INF(NOV),ST34,0.5D0,1.D0)
        ELSEIF(INF_POT(NOV).EQ.2) THEN
          NNC=NLS_NN
          IF(IST.EQ.0) GOTO 1003 
          DO I=1,NNC
            VC(I)=VLS_NN(I,IST,ITT)
            RC(I)=RLS_NN(I,IST,ITT)
          ENDDO
          FACT0=FACT0*(-1.D0)**(S_INF(NOV)+ST34)
     &         *DSQRT(6.D0*(2.D0*ST12+1.D0)*(2.D0*ST34+1.D0))
     &         *SIXJ_D(1.D0,ST12,S_INF(NOV),ST34,1.D0,1.D0)
     ...
     &         *SIXJ_D(1.D0,ST12,S_INF(NOV),ST34,1.D0,1.D0)
        ...
        KLS2(8)=1
      DO 1004 IIV=1,NNC
        IF(VC(IIV).EQ.0.D0) GOTO 1004
        VVC=VC(IIV)
        RRC=RC(IIV)
      DO 1005 NOB=NBMIN,NBMAX
        FACT3=SVFAC(IBOXA,IST,NOA,NOV)*SVFAC(IBOXB,IST,NOB,NOV)
     &       *TVFAC(IBOXA,ITT,NOA,NOV)*TVFAC(IBOXB,ITT,NOB,NOV)
     &       *PH(NOB)
        IF(PH(NOB).EQ.0.D0) GOTO 1005
        IF(FACT3.EQ.0.D0)   GOTO 1005
        NO=0
        DO NFA=1,NFAMAX
          NFBMX=NFBMAX
          IF(IBOXA.EQ.IBOXB) NFBMX=NFA
